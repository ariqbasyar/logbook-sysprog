...
- Persiapkan dependencies
    ...
    berjalan mulus:
    ```shell
    sudo apt-get install kernel-package build-essential linux-source bc kmod
    cpio flex bison libncurses5-dev libelf-dev libssl-dev
    ```
...
- Clean before make
    ...
    Lakukan cleaning atau sebelum melakukan compile lagi agar instalasi lebih
...
- Kompilasi kernel
    ...
    ```shell
    make-kpkg -j 4 --initrd --append-to-version=deb-aw-version kernel_image kernel_headers
    ```
    ...
    - Option --append-to-version
        ...
        nama beru, saya berikan nama "deb-aw-version" berarti nama kernel yang
        baru akan menjadi "4.15.1deb-aw-version" atau dengan kata lain akan
...
- Instalasi kernel
    ...
    ```shell
    dpkg -i linux-headers-4.15.1deb-aw-version_4.15.1deb-aw-version-10.00.Custom_amd64.deb
    ```

    Untuk install linux kernel image nya:
    ```shell
    dpkg -i linux-image-4.15.1deb-aw-version_4.15.1deb-aw-version-10.00.Custom_amd64.deb
    ```
...
- Instalasi Module
    ...
    Dan terlihat di sana terdapat file `initrd.img-4.15.1deb-aw-version`
...
- Update initramfs
    ...
    initramfs nya
    ```shell
    update-initramfs -c -k vmlinuz-4.15.1deb-aw-version
    ```
...
- Pilih kernel di boot selanjutnya
    ...
    Nah di pilihan di sana saya dapat memilih kernel linux 4.15.1deb-aw-version
